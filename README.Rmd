...
Once we have the code for the CWAC site, we can download the count data collected
...
We can do all this using a [dplyr](https://dplyr.tidyverse.org/) approach.
...
You may get some warnings from the parser because data wasn't entered in a
standard format. Make sure your analysis won't be impacted by this.
...
The package `ABDtools` builds upon the [`rgee`](https://github.com/r-spatial/rgee),
...
correspond to the longitude, latitude of the centroid of the CWAC site were the
...
in the dataframe only those variables, that you will be needing for your GEE your analysis.
Remember to always include an identifier field that allows you to join the results
back to you original data, in case you need to. In this case, we will leave only the start
...
For example, let's annotate our counts with [surface water occurrence](https://developers.google.com/earth-engine/datasets/catalog/JRC_GSW1_3_GlobalSurfaceWater),
...
GEE calls this image stacks image 'collections'. Again, we can check
...
Say we are interested in the mean minimum temperature in year 2010. For this we could use
...
We already did some of this at the beginning (please check the section 'Uploading data to GEE' if you don't know what
we are talking about), but we will need to adjust our data slightly to work with TerraClimate data.
...
the 15th of the month they will be matched against the next month, because they are closer in time.

Each image collection its own convention, and we must check what is appropriate in each
...
annotate these data with TerraClimate's monthly minimum temperature data.
...
then annotate with all bands at once. This way you minimize the traffic between your
